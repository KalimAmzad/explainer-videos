#!/usr/bin/env node
/**
 * CLI entry point for LangGraph v9 — Scene-Coder-First Pipeline.
 *
 * Usage:
 *   node scripts/langgraph-v9/index.mjs "Topic Name" [--duration=60] [--scenes=5] [--audience="..."] [--instructions="..."] [--output=path] [--thread=id]
 */
import fs from 'fs';
...
if (!topic) {
  console.error('Usage: node scripts/langgraph-v9/index.mjs "Topic Name" [--duration=60] [--scenes=5] [--audience="..."]');
  process.exit(1);
}
...
function nextOutputDir(base) {
  ...
  let n = 1;
  while (fs.existsSync(path.join(base, `${slug}-v9-run${n}`))) n++;
  return path.join(base, `${slug}-v9-run${n}`);
}
const outputDir = nextOutputDir(PATHS.output);
const threadId  = getArg('--thread=') || `v9-${slug}-${Date.now()}`;

// --- Banner ---
console.log('╔══════════════════════════════════════════════════════╗');
console.log('║  LangGraph v9 — Scene-Coder-First Pipeline           ║');
console.log('╚══════════════════════════════════════════════════════╝');
console.log();
...
try {
  ...
  console.log(`  Narrations: ${result.narrations?.filter(n => n.filePath).length || 0} audio files`);
  ...
  console.error(`\nTo resume: node scripts/langgraph-v9/index.mjs "${topic}" --thread=${threadId}`);
  process.exit(1);
}
