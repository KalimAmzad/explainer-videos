 ...
 *
 * Adapted from v7 — identical functionality, reads from researchNotes.
 *
...
export async function narrationGeneratorNode(state) {
  ...
  try {
    ...
    return { narrations: [{ sceneNumber, filePath: outputPath, duration: result.duration, text: narrationText }] };
  } catch (err) {
    ...
    return { narrations: [{ sceneNumber, filePath: null, duration: estimatedDuration, text: narrationText }] };
  }
}

async function generateTTS(text, outputPath) {
  const apiKey = KEYS.gemini;
  if (!apiKey) throw new Error('GEMINI_API_KEY not set — cannot generate TTS');
