/**
 * Node 4: Scene Compiler (v3.2)
 * Deterministic — assembles all scene outputs into final runnable HTML.
 * No LLM calls. Concatenates SVG defs, SVG bodies, and JS code blocks.
 *
 * v3.2: Arrow markers, addRoughShape/showScene/hideScene helpers,
 * font loading via document.fonts.ready, improved replay reset.
 */
import fs from 'fs';
...
/**
 * Sanitize SVG content from LLM output:
 * - Fix truncated path d attributes
 */
function sanitizeSvg(svg) {
  if (!svg) return '';
  let fixed = svg.replace(/ d='([^']*)\.\.\.'/, " d='M 0 0'");
  fixed = fixed.replace(/ d="([^"]*)\.\.\."/g, ' d="M 0 0"');
  return fixed;
}

/**
 * Generate arrow marker definitions for common colors.
 */
function buildArrowMarkers() {
  ...
  return Object.entries(colors).map(([key, color]) =>
    `<marker id="arrow_${key}" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
    <path d="M 0 0 L 10 5 L 0 10 Z" fill="${color}"/>
  </marker>`
  ).join('\n  ');
}

export async function sceneCompilerNode(state) {
  console.log('\n══════════════════════════════════════');
  console.log('  Node 4: Scene Compiler (v3.2)');
  console.log('══════════════════════════════════════');

  const { sceneOutputs, researchNotes, outputDir, slug, assetDefs } = state;
  const TOTAL = researchNotes.total_duration || 60;
  const scenes = researchNotes.scenes || [];

  // Sort scene outputs by sceneNumber
  const sorted = [...sceneOutputs].sort((a, b) => a.sceneNumber - b.sceneNumber);

  console.log(`  Assembling ${sorted.length} scenes into HTML...`);

  // Arrow marker definitions
  const arrowMarkers = buildArrowMarkers();

  // Merge SVG defs (clip paths from each scene)
  const allSceneDefs = sorted.map(s => s.svgDefs || '').join('\n');
  ...
  const sceneData = scenes.map(s => ({
    ...
    title: s.title,
    time: s.time_start || 0,
    color: s.concept_color || researchNotes.color_palette?.[`scene${s.scene_number}`] || '#2b7ec2',
  }));
...
<title>${esc(researchNotes.topic)} — Whiteboard Explainer</title>
<style>
...
<defs>
  ...
  <!-- Arrow markers for common colors -->
  ${arrowMarkers}
${assetDefs || ''}
${allSceneDefs}
</defs>
...
document.fonts.ready.then(() => {
  ...
  // ═══════════════════════════════════════
  // Helper functions (used by scene JS code)
  // ═══════════════════════════════════════

  /** Stroke draw-on animation using fromTo for seek correctness */
  function drawOn(id, dur, ease) {
    const el = typeof id === 'string' ? document.getElementById(id) : id;
    if (!el) return gsap.to({}, { duration: 0.01 });
    if (typeof DrawSVGPlugin !== 'undefined') {
      return gsap.fromTo(el, { drawSVG: '0%' }, { drawSVG: '100%', duration: dur, ease: ease || 'power1.inOut' });
    }
  ...
  /** Clip-rect text wipe — left-to-right reveal using fromTo */
  ...
  /** Fade-in — ONLY for Rough.js shapes and PNG icons */
  function fadeIn(sel, dur) {
    return gsap.fromTo(sel, { opacity: 0 }, { opacity: 1, duration: dur || 0.5, ease: 'power2.out' });
  }

  /** Draw-on all children of a group (stroke-by-stroke animation) */
  function drawOnGroup(groupId, dur) {
    const group = document.getElementById(groupId);
    if (!group) return gsap.to({}, { duration: 0.01 });
    // First make the group visible
    gsap.set(group, { opacity: 1 });
    const children = group.querySelectorAll('path, line, circle, ellipse, polyline, polygon, rect');
    ...
    children.forEach((child, i) => {
      if (child.getTotalLength) {
        ...
        mini.fromTo(child,
          { strokeDasharray: len, strokeDashoffset: len },
          { strokeDashoffset: 0, duration: Math.max(0.2, perChild), ease: 'power1.inOut' },
          i * perChild * 0.7
        );
      } else {
        mini.fromTo(child,
          { opacity: 0 },
          { opacity: 1, duration: Math.max(0.2, perChild), ease: 'power2.out' },
          i * perChild * 0.7
        );
      }
    });
  ...
  /** Add a Rough.js generated shape to a scene group */
  function addRoughShape(roughNode, id, parentSceneId) {
    if (!roughNode) return;
    roughNode.setAttribute('id', id);
    gsap.set(roughNode, { opacity: 0 });
    const parent = document.getElementById(parentSceneId);
    if (parent) {
      parent.appendChild(roughNode);
    } else {
      document.getElementById('board').appendChild(roughNode);
    }
  }
  ...
  // ── Scene timeline code (generated by scene coders) ──
${allJsCode}
  ...
  function doReplay() {
    ...
    document.querySelectorAll('path[id], line[id], polyline[id]').forEach(el => {
      if (el.getTotalLength) {
        const len = el.getTotalLength();
        gsap.set(el, { strokeDasharray: len, strokeDashoffset: len });
      }
    });
    // Reset Rough.js shapes
    document.querySelectorAll('[id^="rough_"]').forEach(el => {
      gsap.set(el, { opacity: 0 });
    });
    // Reset illustration groups
    document.querySelectorAll('[id*="_illust_"]').forEach(el => {
      gsap.set(el, { opacity: 0 });
      el.querySelectorAll('path, line, circle, ellipse, polyline, polygon, rect').forEach(child => {
        if (child.getTotalLength) {
          const len = child.getTotalLength();
  ...
  console.log(`  Duration: ${TOTAL}s, Scenes: ${sorted.length}`);

  return {
    finalHtml: html,
    outputPath,
    currentStep: 'compilation_complete',
  };
}
